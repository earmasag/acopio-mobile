...
import type { ReceivedBox } from "@/types/receive";

type ReceiveBoxListProps = {
  boxes: ReceivedBox[];
  onRemove?: (boxId: string) => void;
  readOnly?: boolean;
};

export function ReceiveBoxList({
  boxes,
  onRemove,
  readOnly = false,
}: ReceiveBoxListProps) {
  ...
  function handleRemove(box: ReceivedBox) {
    if (!canRemove || !onRemove) return;

    Alert.alert(
      "Quitar caja",
      `¿Quitar la caja ${shortUuid(box.packageUuid)} de esta recepción?`,
      [
  ...
  if (boxes.length === 0) {
    return (
      <View className="items-center rounded-2xl border border-dashed border-amber-300 bg-white/70 px-6 py-8">
        <MaterialIcons name="inventory-2" size={36} color="#52796F" />
        <Text className="mt-3 text-center text-sm text-acopio-muted">
          {readOnly
            ? "Esta recepción no registró cajas."
            : "Aún no hay cajas recibidas. Escanea el QR de cada caja al descargarla."}
        </Text>
      </View>
    );
  }
  ...
  return (
    <View className="gap-2">
      {boxes.map((box, index) => (
        <View
          key={box.id}
          className="flex-row items-center gap-3 rounded-xl border border-amber-200 bg-white px-3 py-3"
        >
          <View className="h-8 w-8 items-center justify-center rounded-lg bg-amber-100">
            <Text className="text-sm font-bold text-amber-800">{index + 1}</Text>
          </View>
          ...
          {canRemove && (
            <Pressable
              accessibilityLabel="Quitar caja de la recepción"
